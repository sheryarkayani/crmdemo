...
import { MoreHorizontal, Building, Users, TrendingUp, DollarSign } from 'lucide-react';
...
const AccountsDashboard = () => {
  ...
  const activeClients = allTasks.filter(task => task.status === 'Active Client').length;
    ...
    <div className="p-6 bg-background space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Client and account management analytics</p>
        </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-orange-600">
                  {allTasks.length > 0 ? Math.round((activeClients / allTasks.length) * 100) : 0}%
                </p>
              </div>
            ...
            {currentBoard.groups?.map((group, index) => (
              <div key={index} className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <Badge variant="outline">{group.tasks?.length || 0} accounts</Badge>
                </div>
